...
router.post('/signin', (req, res) => {
    ...
    .then(user => {
        if(!user) res.status(404).json({error: 'The email adress or the password do not match !'})
        else {
            bcrypt.compare(req.body.password, user.password, (error, match) => {
                ...
                else res.status(403).json({error: 'The email adress or the password do not match !'})
            })
        }
    })
    .catch(error => {
